# Air Cargo Analysis, Full SQL Script

Every task in this project, written out in order, each one runnable on its own.

```sql
-- Air Cargo Analysis
-- Full project script, all 19 tasks in a single file
--
...
Import customer.csv, routes.csv, ticket_details.csv and passengers_on_flights.csv from the Datasets folder into the four tables above using the Table Data Import Wizard before running the tasks below. Date fields are stored in DD-MM-YYYY format in the CSV files and should be converted to DATE type on import.
